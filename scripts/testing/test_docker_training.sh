...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Testing Training Container Locally${NC}"
echo -e "${GREEN}========================================${NC}"

# Create local directories for input/output
echo -e "\n${YELLOW}1. Setting up local directories...${NC}"
mkdir -p local_test/data
mkdir -p local_test/models
mkdir -p local_test/logs

# Copy generated data
echo -e "\n${YELLOW}2. Copying test data...${NC}"
if [ -f "customer_engagement_dataset_extended.parquet" ]; then
    cp customer_engagement_dataset_extended.parquet local_test/data/
    echo -e "${GREEN}✓ Parquet file copied (18.7 MB)${NC}"
else
    echo -e "${RED}✗ Data file not found!${NC}"
    exit 1
fi

# Create a simplified training script for testing
echo -e "\n${YELLOW}3. Creating test training script...${NC}"
cat > local_test/train_simple.py << 'EOF'
import pandas as pd
import numpy as np
...
echo -e "\n${YELLOW}4. Running training container...${NC}"
echo -e "   ${YELLOW}(This will take ~30-60 seconds for 100K rows)${NC}"

docker run --rm \
    -v "$(pwd)/local_test/data:/data" \
    -v "$(pwd)/local_test/models:/models" \
    -v "$(pwd)/local_test/train_simple.py:/app/train_simple.py" \
    engagement-training:latest \
    python train_simple.py

# Check results
echo -e "\n${YELLOW}5. Checking results...${NC}"
if [ -f "local_test/models/metrics.json" ]; then
    echo -e "${GREEN}✓ Training completed successfully!${NC}"
    echo -e "\n📊 Metrics:"
    cat local_test/models/metrics.json | python3 -m json.tool
else
    echo -e "${RED}✗ Training failed - no metrics found${NC}"
    exit 1
fi

# List generated models
echo -e "\n${YELLOW}6. Generated models:${NC}"
ls -lh local_test/models/

echo -e "\n${GREEN}========================================${NC}"
echo -e "${GREEN}✅ Training container test complete!${NC}"
echo -e "${GREEN}========================================${NC}"
